...
export default function balance8Columnas() {
  ...
  const [dataBalance8ColumnasClasificado, setDataBalance8ColumnasClasificado] = useState([]);

  useEffect(() =>{
    setEmpresa_name(String(secureLocalStorage.getItem(SESSION_NAMES.EMPRESA_NAME))!.replace(/"/g, ''));
  }),[];

  const columnsBalance8ColumnasClasificado: TableColumn<any>[] = [
    {
      id: 'cuenta',
      name: 'Cuenta',
      sortable: true,
  ...
  const handleSubmitParams = () => {
    ...
    axios
      .get(API_CONTABILIDAD + "/Balance8Columnas/clasificado", { params: params, headers: getHeaders() })
      .then((response) => {

        setDataBalance8ColumnasClasificado(response.data);
  ...
  return (
    <section>
      <h1 className="dark:text-white text-xl mb-4">Balance 8 Columnas Clasificado dentro de la empresa {empresa_name}</h1>

      <div className="flex my-4 items-center gap-2">
        <label className="block ml-4 dark:text-white text-gray-700 text-sm font-bold"
          htmlFor="fechares">Fecha Corte</label>
      ...
      <DataTable
        columns={columnsBalance8ColumnasClasificado}
        data={dataBalance8ColumnasClasificado}
        customStyles={gridStyle}
        dense
        expandableRows
